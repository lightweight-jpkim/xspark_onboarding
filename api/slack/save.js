// Vercel Serverless Function - Slack 일일 리포트를 Notion에 저장
import { NotionService } from '../../backend/services/notion.js';
import appConfig from '../../backend/config/index.js';

const notionService = new NotionService(process.env.NOTION_API_TOKEN || appConfig.notion.apiToken);
...
export default async function handler(req, res) {
  ...
  try {
    const { title, content, parentPageId, channelInfo } = req.body;
    ...
    console.log('📝 Notion에 Slack 리포트 저장 시작...');
    ...
    const page = await notionService.client.pages.create({
      ...
      properties: {
        ...
        '날짜': {
          date: {
            start: channelInfo?.date || new Date().toISOString().split('T')[0]
          }
        }
      }
    });
